...
SECTIONS.append(arch_section(
    ...
    Resultado: <strong>0.794 ± 0.033</strong> (6 seeds); 0.816 con 1 cabeza — y el <strong>campeón global
    es este mismo modelo con encoding ORDINAL</strong> de categóricas: <strong>0.824 ± 0.018</strong>
    (<code>feat_ordinal</code>: cada nivel entra como su rango de BTR en train).'''))
...
SECTIONS.append(arch_section(
    ...
    <code>text_words</code> (tokenización por palabras, 64 tokens) corre en la 3ª tanda.'''))
...
SECTIONS.append(arch_section(
    ...
    medido: −0.06). Variantes: tokenización por <strong>palabras</strong> (64 tokens; "(Best Seller)" son 2
    palabras) y embeddings <strong>pre-entrenados con word2vec</strong> sobre train vs end-to-end — la
    comparación clase 1 vs clase 2. Resultados: 3ª tanda en la 3070.'''))
...
SECTIONS.append(arch_section(
    ...
    adentro del token de producto (<code>listwise_texto</code>, propuesta de Junior): <strong>0.753</strong>
    (+0.016 en 2/2 seeds) — estaba parcialmente ciego, no mal concebido; pero sigue lejos del tabular: la
...
<section class="card" id="tabla">
  ...
  <tr><td>5B · fusión</td><td>feature o RESUMEN del texto</td><td class="n">15</td><td>features ↔ resumen</td>
      <td class="n">63.969</td><td class="n">3ª tanda</td><td class="c">--formulation fusion</td></tr>
  ...
  <strong>supera al GBM y al MLP</strong> (atención: +0.048 apareado, gana 5/6 seeds) y el campeón global es
  <strong>0.824 ± 0.018</strong> (<code>feat_ordinal</code>: categóricas como su rango de BTR en train, paciencia
  20; con embeddings, <code>pac20_feat_h1</code> 0.816). Ninguna variante con texto supera al tabular puro; el
...
<code>btr/data.py</code> · <code>experimentos.py</code> — actualizado tras la 2ª tanda GPU (17/08/2026).
