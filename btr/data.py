...
class Preprocessor:
    ...
    def cat_cardinalities(self):
        return [len(self.vocabs[f]) + 1 for f in CAT_FEATURES]  # +1 por UNK

    def transform(self, df):
        """df -> (x_cat long, x_num float32, x_text long, y float32) listos para el modelo."""
        x_cat = np.stack(
            [df[f].map(lambda v, f=f: self.vocabs[f].get(v, 0)).to_numpy() for f in CAT_FEATURES],
            axis=1,
        )
...
def make_query_tensors(df, prep, max_products=None):
    ...
    """
    x_cat, x_num, _, y = prep.transform(df)
    ...
    return out_cat, out_num, out_mask, out_y


def prepare_listwise(csv_path, seed=42):
    ...
    return prep, max_products, {
        'train': make_query_tensors(train_df, prep, max_products),
        'val': make_query_tensors(val_df, prep, max_products),
        'test': make_query_tensors(test_df, prep, max_products),
    }
